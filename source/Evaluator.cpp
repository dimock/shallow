...
Evaluator::PasserInfo passerEvaluation(Board const& board, const Evaluator::FieldsInfo(&finfo)[2], BitMask const mask_all, Evaluator::PasserInfo const& pi)
{
  ...
  while (psmask) {
    ...
    bool neighbours = (pawnMasks().mask_neighbor(color, n) & pmask & pi.passers_) != 0ULL;
    bool guarded = !neighbours &&
      ( ((finfo[color].pawnAttacks_ & my_field) != 0ULL) || ((pawnMasks().mask_neighbor(color, n) & pmask & ~pi.passers_) != 0ULL) );
    ...
    if (passmsk & opmsk) {
      ...
      pwscore += EvalCoefficients::passerPawnGrd2_[cy] * guarded;
      pwscore +=
      ...
      pwscore += EvalCoefficients::passerPawnNbs_[cy] * neighbours;
      pwscore += EvalCoefficients::passerPawnGrd_[cy] * guarded;
      pwscore +=
...
ScoreType32 Evaluator::evaluateAttacks(Figure::Color color)
{
  ...
  if (auto bi_treat = (o_rq_mask & finfo_[color].bi_treat_ & ~counted_mask)) {
  ...
  if (auto qr_attack = (fmgr.rook_mask(ocolor) & finfo_[color].qbi_treat_ & qr_possible_mask)) {
    counted_mask |= qr_attack;
    ++attackedN;
    attackScore += EvalCoefficients::rookQueenAttackedBonus_;
  }

  if (auto r2q_treat = (fmgr.queen_mask(ocolor) & finfo_[color].r_treat_ & ~counted_mask)) {
