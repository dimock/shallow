...
ScoreType32 Evaluator::evaluateKingPressure(Figure::Color color)
{
  ...
  if (near_oking_att) {
    attack_coeff += EvalCoefficients::attackedNearKingCoeff_ * pop_count(near_oking_att);
  }
...
//  }
//  ScoreType32 score{ finfo_[color].score_opening_, 0 };
...
//      continue;
//    finfo_[color].discoveredCheck_ = true;
//    score += EvalCoefficients::discoveredCheckBonus_;
//    if (mpattack & finfo_[ocolor].nbrq_mask_ & (finfo_[color].attack_mask_ | ~finfo_[ocolor].kingAttacks_)) {
//      score += EvalCoefficients::discoveredCheckBonus4_;
//    }
//    break;
//  }
//
...
//                      finfo_[color].discoveredCheck_* EvalCoefficients::discoveredCheckBonus1_;
//
//    if (could_be_check) {
