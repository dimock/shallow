...
struct ChecksGenerator
{
  ...
  void generateMates(bool moveFound)
  {
    ...
    auto oki_moves = movesTable().caps(Figure::TypeKing, oki_pos) & ~blocked_mask;
    ...
    if(!moveFound && moves_.empty())
    {
      ...
      for (; pw_check_mask && moves_.empty();)
      {
        ...
        for (; pw_from;)
        {
          ...
          if (board_.is_something_between(from, to, mask_all_inv))
            continue;
          if (oki_moves == 0ULL || (!moveFound && moves_.empty() && (o_kbrq_mask & movesTable().pawnCaps(color, to)) != 0ULL))
            add(from, to);
          break;
        }
      }
      ...
      for (; kn_mask && !found;)
      {
        ...
        for (; kn_moves;)
        {
          ...
          if ((!oki_moves || discovered && !can_escape) ||
              (!moveFound && moves_.empty() && (o_brq_mask & movesTable().caps(Figure::TypeKnight, to)) != 0ULL))
          {
            add(from, to);
            found = true;
          }
